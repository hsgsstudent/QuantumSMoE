...
def train(model, device, train_loader, optimizer, epoch, LR):
    ...
    for batch_idx, (x, z, y, magnitude, syndrome) in enumerate(train_loader):
        ...
        #.
        loss = args.lambda_loss_ber*loss1 + args.lambda_loss_ler*loss3 + args.lambda_ortho_loss*o_loss
    ...
    with torch.no_grad():
        for ii, test_loader in enumerate(test_loader_list):
            ...
            while True:
                ...
                z_pred, o_loss = model(syndrome.to(device))
...
def main(args):
    ...
    train_dataloader = DataLoader(QECC_Dataset(code, ps_train, len= 128 * 5000, args=args), batch_size=int(args.batch_size),
    ...
    #     torch.save(model, os.path.join(args.path, 'last_model'))
        if loss < best_loss:
            ...
            torch.save(
                ...
                os.path.join(args.path, 'best_model.pt')
            )
            logging.info(f'Model saved at epoch {epoch} (loss={loss:.4f})')
    #     if epoch % 60 == 0 or epoch in [1, args.epochs]:
    #         test(model, device, test_dataloader_list, ps_test)
    # ###
    checkpoint = torch.load(os.path.join(args.path, 'best_model.pt'), map_location=device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--workers', type=int, default=0) # origin is 4
    ...
    parser.add_argument('--code_L', type=int, default=8,help='Lattice length')
    ...
    args.code = code
    ####################################################################
    model_dir = os.path.join('Data','Results_QECCT(2last-ViTsoftMoE-128d-8e-4s-5000batch-softmax-AdamW-o_loss1-0.05)', args.code_type, 
